...
cp ./arxiv_keywords.txt  /mnt/Vancouver/apps/arxiv
cp ./sed_characters /mnt/Vancouver/apps/arxiv
...
curl -s http://export.arxiv.org/rss/$1 > .arxiv-temp                         ## -s : silent ; > : write to/overwrite existing temp file
...
if [[ "$cs_AI_TIMESTAMP_INT" -eq "$OLD_DATE_INT" ]]; then
  echo 'No new articles RSS feeds.' 2>&1 | tee -a log
else
  echo 'Retrieving new articles RSS feeds ...' 2>&1 | tee -a log
  # >> : append to file
